...
for i, n_s in enumerate(n_s_ls):
    ...
    obj = obj[:,:,0]
    smse_normal = (k**2)*np.sum(finite_support*(grid_delta - obj)**2)/n_sample_pixel
    smse_normal_ls.append(smse_normal)

np.savez(os.path.join(save_path, "smse"), n_s_ls, smse_normal_ls)
...
ax.set_ylabel('SMSE', fontsize=Fontsize)
